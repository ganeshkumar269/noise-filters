...
namespace mysf {
    ...
    double variance_MT(sf::Image& image,sf::IntRect dimension){
        ...
        std::future<double> t1 = std::async(std::launch::async, variance,std::ref(image),dim1);
        std::future<double> t2 = std::async(std::launch::async, variance,std::ref(image),dim2);
        std::future<double> t3 = std::async(std::launch::async, variance,std::ref(image),dim3);
        std::future<double> t4 = std::async(std::launch::async, variance,std::ref(image),dim4);
